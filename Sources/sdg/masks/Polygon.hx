...
class Polygon extends Hitbox
{
	...
	private var _fakeObject:Object;				// used for Grid and Pixelmask collision
	...
	*/
	...
	/**
	 * Checks for collisions with an Object.
	 */
	...
	private function collideGrid(grid:Grid):Bool
	{
		...
		_fakeObject.width = tileW;
		_fakeObject.height = tileH;
		_fakeObject.x = _parent.x;
		_fakeObject.y = _parent.y;
		_fakeObject.originX = grid._parent.originX + grid._x;
		_fakeObject.originY = grid._parent.originY + grid._y;

		_fakeTileHitbox._width = tileW;
		_fakeTileHitbox._height = tileH;
		_fakeTileHitbox._parent = _fakeObject;

		for (r in 0...grid.rows)
		{
			for (c in 0...grid.columns)
			{
				_fakeObject.x = grid._parent.x + grid._x + c * tileW;
				_fakeObject.y = grid._parent.y + grid._y + r * tileH;
